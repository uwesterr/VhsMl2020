...
A good explanation of the theory behind SVMs is given in [@9781461471370]
...
Notice that in \@ref(eq:9-18), in order to evaluate the function f(x), we need to compute the inner product between the new point *x* and each of the training points $x_i$. However, it turns out that $α_i$ is nonzero only for the support vectors in the solution—that is, if a training observation is not a support vector, then its $α_i$ equals zero. So if *S* is the collection of indices of these support points, we can rewrite any solution function of the form \@ref(eq:9-18) as

\begin{equation} 
f(x)=\beta_{0}+\sum_{i \in \mathcal{S}} \alpha_{i}\left\langle x, x_{i}\right\rangle
  (\#eq:9-19)
\end{equation}
which typically involves far fewer terms than in \@ref(eq:9-18)

### Kernels

To summarize, in representing the linear classifier $f (x)$, and in computing its coefficients, all we need are inner products. Now suppose that every time the inner product \@ref(eq:9-17) appears in the representation \@ref(eq:9-18), or in a calculation of the solution for the support vector classifier, we replace it with a generalization of the inner product of the form
...
which would just give us back the support vector classifier. Equation \@ref(eq:9-21) is known as a linear kernel because the support vector classifier is linear in the features; the linear kernel essentially quantifies the similarity of a pair of observations using Pearson (standard) correlation. 

#### Polynomial Kernel  

But one could instead choose another form for \@ref(eq:9-20). For instance, one could replace every instance of $\sum_{j=1}^{p} x_{i j} x_{i^{\prime} j}$ with the quantity
...
The polynomial kernel shown in \@ref(eq:9-22) is one example of a possible non-linear kernel, but alternatives abound. 
...
In \@ref(eq:9-24), γ is a positive constant.

How does the radial kernel \@ref(eq:9-24) actually work? If a given test observation $x^{*}=\left(x_{1}^{*} \ldots x_{p}^{*}\right)^{T}$ is far from a training observation $x_i$ in terms of Euclidean distance, then $\sum_{j=1}^{p}\left(x_{j}^{*}-x_{i j}\right)^{2}$ will be large, and so  $K\left(x_{i}, x_{i^{\prime}}\right)={\exp \left(-\gamma \sum_{j=1}^{p}\left(x_{j}^{*}-x_{i j}\right)^{2}\right)}$ will be very tiny. This means that in \@ref(eq:9-23), $x_{i}$ will play virtually no role in $f\left(x^{*}\right)$. Recall that the predicted class label for the test observation $x^{*}$ is based on the sign of $f\left(x^{*}\right)$. In other words, training observations that are far from $x^{*}$ will play essentially no role in the predicted class label for $x^{*}$. This means that the radial kernel has very local behaviour, in the sense that only nearby training observations have an effect on the class label of a test observation.
...
advantage is computational, and it amounts to the fact that using kernels, one need only compute $K\left(x_{i}, x_{i^{\prime}}\right)$ for $\left( \begin{array}{l}{n} \\ {2}\end{array}\right)$  distinct pairs $i$, $i^{\prime}$.This can be
done without explicitly working in the enlarged feature space. This is im- portant because in many applications of SVMs, the enlarged feature space is so large that computations are intractable. For some kernels, such as the radial kernel \@ref(eq:9-24), the feature space is *implicit* and infinite-dimensional, so we could never do the computations there anyway! 
...
![Figure from https://blogs.nvidia.com/wp-content/uploads/2018/09/autos-672x378.png](https://blogs.nvidia.com/wp-content/uploads/2018/09/autos-672x378.png){width=100% .external}
...
The service is available at https://datasetsearch.research.google.com

In a blog published Jan 23, 2020 Google states 
<blockquote>
